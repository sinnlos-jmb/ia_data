...
// Build SQL query dynamically
  ...
  const query = `
    SELECT id, disease, metadata, symptoms,
...
const args = process.argv.slice(2).join(", "); // Ignore the first two elements
console.log('Arguments:', args);

// Parsing specific arguments
if (args.length > 0) {
//    await querySimilar("chest pain and reduced blood flow to the heart");
//    await querySimilar("irregular heartbeet, arrhythmia");
    await querySimilar(args);
} else {
